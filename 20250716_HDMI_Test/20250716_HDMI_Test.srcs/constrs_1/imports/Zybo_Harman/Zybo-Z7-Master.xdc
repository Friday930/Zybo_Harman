...
## 추가 클럭 및 타이밍 제약 조건
# TMDS 클럭 생성 (25MHz)
create_clock -add -name tmds_clk -period 40.00 -waveform {0 20} [get_nets tmds_clk_p]

# OV7670 PCLK 클럭 (25MHz)  
create_clock -add -name ov7670_pclk_clk -period 40.00 -waveform {0 20} [get_ports ov7670_pclk]

# 클럭 도메인 간 비동기 경로 설정
set_clock_groups -asynchronous -group [get_clocks sys_clk_pin] -group [get_clocks tmds_clk] -group [get_clocks ov7670_pclk_clk]

# OV7670 PCLK 클럭 라우팅 경고 무시
set_property CLOCK_DEDICATED_ROUTE FALSE [get_nets ov7670_pclk_IBUF]

# 입출력 지연 설정
set_input_delay -clock [get_clocks ov7670_pclk_clk] -min 2.000 [get_ports ov7670_*]
set_input_delay -clock [get_clocks ov7670_pclk_clk] -max 8.000 [get_ports ov7670_*]

set_output_delay -clock [get_clocks tmds_clk] -min 1.000 [get_ports tmds_*]
set_output_delay -clock [get_clocks tmds_clk] -max 2.000 [get_ports tmds_*]
# HDMI TX 출력 (Zybo Z7 HDMI TX 커넥터 - TMDS 차동 신호)
set_property -dict { PACKAGE_PIN H16   IOSTANDARD}
# Zybo Z7 HDMI 제약 조건 파일 (공식 제약 파일 기준)
...
# OV7670 카메라 인터페이스 (이미 제약 파일에 정의된 핀 사용)
# Pmod JB 핀들 (공식 제약 파일과 동일)
set_property -dict { PACKAGE_PIN W6    IOSTANDARD LVCMOS33 } [get_ports ov7670_xclk]     
set_property -dict { PACKAGE_PIN W8    IOSTANDARD LVCMOS33 } [get_ports ov7670_pclk]     
set_property -dict { PACKAGE_PIN V6    IOSTANDARD LVCMOS33 } [get_ports ov7670_href]    
set_property -dict { PACKAGE_PIN U7    IOSTANDARD LVCMOS33 } [get_ports ov7670_v_sync]  

# OV7670 데이터 버스 [7:0] (Pmod JC - 공식 제약 파일과 동일)
set_property -dict { PACKAGE_PIN U12   IOSTANDARD LVCMOS33 } [get_ports {ov7670_data[0]}]
set_property -dict { PACKAGE_PIN T10   IOSTANDARD LVCMOS33 } [get_ports {ov7670_data[1]}]
set_property -dict { PACKAGE_PIN T12   IOSTANDARD LVCMOS33 } [get_ports {ov7670_data[2]}]
set_property -dict { PACKAGE_PIN T11   IOSTANDARD LVCMOS33 } [get_ports {ov7670_data[3]}]
set_property -dict { PACKAGE_PIN Y14   IOSTANDARD LVCMOS33 } [get_ports {ov7670_data[4]}]
set_property -dict { PACKAGE_PIN W15   IOSTANDARD LVCMOS33 } [get_ports {ov7670_data[5]}]
set_property -dict { PACKAGE_PIN W14   IOSTANDARD LVCMOS33 } [get_ports {ov7670_data[6]}]
set_property -dict { PACKAGE_PIN V15   IOSTANDARD LVCMOS33 } [get_ports {ov7670_data[7]}]

# HDMI TX TMDS 차동 출력 (공식 제약 파일에서 그대로 가져옴)
...
# HDMI 제어 신호들 (공식 제약 파일에서 그대로 가져옴)
...
# 타이밍 제약 조건
create_clock -add -name tmds_clk -period 40.00 -waveform {0 20} [get_nets tmds_clk_p]
create_clock -add -name ov7670_pclk -period 40.00 -waveform {0 20} [get_ports ov7670_pclk]

# 클럭 도메인 간 비동기 경로 설정
set_clock_groups -asynchronous -group [get_clocks sys_clk_pin] -group [get_clocks tmds_clk] -group [get_clocks ov7670_pclk]

# 입출력 지연 설정
set_input_delay -clock [get_clocks ov7670_pclk] -min 2.000 [get_ports ov7670_*]
set_input_delay -clock [get_clocks ov7670_pclk] -max 8.000 [get_ports ov7670_*]
